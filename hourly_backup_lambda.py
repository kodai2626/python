...
def lambda_handler(event, context):
    try:
        ...
        # 5分前の時刻を計算（14時時点のバックアップを取得）
        backup_time = now - timedelta(minutes=5)
        export_time = backup_time.isoformat()
        
        logger.info(f"バックアップをエクスポート開始: テーブル {TABLE_NAME}, バックアップ時点: {export_time}")
